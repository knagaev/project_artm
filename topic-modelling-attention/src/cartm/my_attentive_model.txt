...
def local_bidir_ema(
    ...
    # 1. Относительные индексы: [0,1,2,...,L1-1, 0,1,...,L2-1, ...]
    # Экспоненты теперь ограничены max(lengths), а не I
    rel_arange = np.arange(I) - np.repeat(starts, lengths)
    
    # 2. Стабилизация: ограничиваем min значение, чтобы 1/alpha_pow не уходил в inf
    #eps = np.finfo(np.float64).tiny
    eps = 1e-12
    alpha_pow = np.maximum(alpha ** rel_arange, eps)
    alpha_inv_pow = 1.0 / alpha_pow
    ...
    def calc_right_ema(X_in, idx_in):
        ...
        X_starts = np.repeat(X_in[:, starts], lengths, axis=1)
        # На началах сегментов rel_arange == 0, поэтому alpha_inv_pow == 1.0
        alpha_inv_starts = np.repeat(alpha_inv_pow[starts], lengths)
