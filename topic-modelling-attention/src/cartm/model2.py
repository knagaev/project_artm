...
class ContextTopicModel2():
    ...
    def __init__(
        ...
        self._regularizations = {}  # список объектов-функторов, которые на вход получают Фи, на выходе float (меньше - лучше?)
    ...
    def _get_context_tensor(self, *, batch: jax.Array) -> jax.Array:
        ...
        # размножение батча с учетом контекстов - длина батча раз
    ...
    @partial(jax.jit, static_argnums=0)
    def _create_context_weights(self, *, 
        batch_size: int, 
        document_bounds: jax.Array, 
    ) -> jax.Array:

        # веса контекста с затуханием
        suffix_weights = np.cumprod(np.full(self.ctx_len, (1 - self._gamma))) * self._gamma
        prefix_weights = suffix_weights[::-1]
        center_weight =  np.array([self._gamma]) if self._self_aware_context else np.array([0.0])
        
        base_weights = np.concatenate([prefix_weights, center_weight, suffix_weights])  # (2C+1,)
        
        # 2. Маска валидности контекста
        positions = jnp.arange(batch_size)
        context_offsets = jnp.arange(-self.ctx_len, self.ctx_len + 1)
        ...
        context_matrix = self._norm((attn_mask * base_weights).T).T
    ...
    def _calc_theta(
        ...
        #)  # (I, 2C + 1)
        context_matrix = self._create_context_weights(
            batch_size=len(batch),
            document_bounds=ctx_bounds,
        )
    ...
    # сборка регуляризаторов в функцию градиента для [1, reg1(x), reg2(x)...]
    ...
    def _batched_step_wrapper( 
        ...
        for batch, ctx_bounds_batch in batches: # для каждого батча вызываем стандартный Е-шаг
            ...
            phi_new = phi_new * (1 - lr) + phi_step * lr    # накопление новой Фи с затуханием
            n_t_new = n_t_new * (1 - lr) + n_t_step * lr    # накопление новой n_t с затуханием
            phi_it.append(phi_it_step)                      # добавляем
            theta.append(theta_step)                        # добавляем в Тета
    ...
    def fit(
        ...
        self.phi = self._norm(self.phi) # нормализуем Фи
        self.n_t = jnp.full(            # вектор-псевдоматрица размерностью (T, ) для хранения частотности топиков
            shape=(self.n_topics, ),    # инициализация равномерным
            fill_value=len(data) / self.n_topics,
        )  # (T, )
        grad_regularization = self._compose_regularizations() # градиенты всех регуляризаторов

        for it in range(max_iter):      # для всех итераций
            if ctx_bounds is None:      # если контекстные окна не заданы
                # batched input
            ...
            if verbose > 0:
                print(f'Iteration [{it + 1}/{max_iter}], _phi update diff norm: {diff_norm:.04f}')
