...
class AttentiveTopicModel:
    ...
    def fit(
        ...
        if self.attention_mode == 'explicit':
            weights_state = self._resolve_explicit_weights(weights_t=weights_state, batch=data)
        ...
        for it in iterator:
            ...
            # счетчики. Это упрощает последующие байесовские пересчеты p_t.
